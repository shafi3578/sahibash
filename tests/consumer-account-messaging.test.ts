...
test("listing pagination is server-side and image viewing supports touch navigation", () => {
  ...
  assert.match(home, /const pageSize = 10/);
  assert.match(home, /getApprovedListings\(\{ locale, limit: pageSize, offset: \(currentPage - 1\) \* pageSize \}\)/);
  assert.match(home, /const \[latest, featured, totalListings, mobileCategories\] = await Promise\.all/);
  assert.doesNotMatch(home, /latestCandidates|featuredIds/);
