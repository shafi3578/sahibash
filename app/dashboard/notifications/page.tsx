...
function destination(payload: unknown) {
  const p = (payload && typeof payload === "object" ? payload : {}) as Record<string, unknown>;
  if (typeof p.listing_id === "string" && typeof p.sender_user_id === "string") return `/dashboard/messages?listing=${encodeURIComponent(p.listing_id)}&participant=${encodeURIComponent(p.sender_user_id)}`;
  if (typeof p.listing_id === "string") return `/listings/${encodeURIComponent(p.listing_id)}`;
  if (typeof p.offer_id === "string") return "/dashboard/offers";
...
export default async function NotificationsPage() {
  ...
  return <DashboardSection currentPath="/dashboard/notifications" title={copy.title} description={copy.description}>
    ...
    {(data ?? []).length === 0 ? <p className="rounded-2xl border border-dashed border-[var(--line)] p-5 text-[var(--ink-2)]">{copy.empty}</p> : <div className="space-y-2">{(data ?? []).map((item) => <article key={item.id} className={`rounded-2xl border p-4 ${item.is_read ? "border-[var(--line)] bg-white" : "border-red-200 bg-red-50"}`}><div className="flex items-start justify-between gap-3"><div><p className="font-bold text-[var(--ink-1)]">{item.title}</p><p className="mt-1 text-sm text-[var(--ink-2)]">{item.body}</p><p className="mt-2 text-xs text-[var(--ink-2)]">{new Date(item.created_at).toLocaleString(locale === "en" ? "en-US" : `${locale}-AF`)}</p></div>{!item.is_read ? <span className="mt-1 h-2.5 w-2.5 shrink-0 rounded-full bg-red-600" aria-label={copy.unread} /> : null}</div><div className="mt-3 flex gap-2"><Link href={localizePath(destination(item.payload), locale)} className="rounded-lg bg-[var(--ink-1)] px-3 py-2 text-xs font-bold text-white">{copy.open}</Link>{!item.is_read ? <form action={markNotificationReadAction}><input type="hidden" name="id" value={item.id}/><button aria-label={copy.allRead} className="rounded-lg border border-[var(--line)] px-3 py-2 text-xs font-bold">✓</button></form> : null}</div></article>)}</div>}
  </DashboardSection>;
}
