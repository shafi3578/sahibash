...
import { configureTelegramImportWebhookAction } from "@/lib/actions/telegram-webhook";
...
export default async function InventoryControlCenterPage() {
  ...
  const copy = locale === "fa"
    ...
    : locale === "ps"
      ? {
          ...
          view: "ریکارډونه وګورئ", sourcesHelp: "تأیید شوې سرچینې چې صاحبش ته اعلانونه لېږي.", jobsHelp: "د هر لېږد حالت، ازمایښتي اجرا او د پروسس پایله.", candidatesHelp: "تر اصلي اعلان جوړېدو مخکې ترلاسه شوي ریکارډونه.", externalHelp: "په بریالیتوب جوړ شوي اعلانونه؛ هر ریکارډ د اعلان پاڼې تړونی لري.", reviewHelp: "د مالکیت او لرې کولو غوښتنې، تکراري موارد او ثبت شوې منع. حساس پرېکړې یوازې د AAL2 MFA سره اجرا کېږي.",
          empty: "تر اوسه هېڅ ریکارډ نشته.", notCreated: "اعلان لا نه دی جوړ شوی", waiting: "لېږد ترلاسه شوی، خو لا عام اعلان نه دی. له بیاکتنې او بریالۍ جوړونې وروسته به یې تړونی همدلته ښکاره شي.", live: "د نویو اعلانونو کتار هر ۳۰ ثانیې کتل کېږي", configureWebhook: "بوټ په خوندي ډول تولید ته ونښلوئ", reviewTransfer: "لېږد بیاکتل", openListing: "اعلان پرانیستل", openSource: "اصلي سرچینه پرانیستل", source: "سرچینه", status: "حالت", created: "جوړ شوی", method: "طریقه", platform: "پلېټفارم", rows: "ریکارډونه", result: "پایله", dryRun: "ازمایښتي اجرا", liveRun: "اصلي اجرا", accepted: "منل شوي", rejected: "رد شوي", errors: "تېروتنې", row: "ریکارډ", sourceItem: "په سرچینه کې پېژند", category: "کټګوري", location: "ځای", price: "بیه", missingCategory: "نه ده ټاکل شوې", missingLocation: "نه دی ټاکل شوی", missingPrice: "نه ده ټاکل شوې", activeSource: "سرچینه فعاله ده", emergencyStop: "بېړنی تم فعال دی", reason: "دلیل", score: "نمره", latest: `وروستي ${RECENT_LIMIT} ریکارډونه ښودل کېږي`, claimsEmpty: "د مالکیت غوښتنه نشته.", removalsEmpty: "د لرې کولو غوښتنه نشته.", duplicatesEmpty: "تکراري مورد نشته.", optOutsEmpty: "منع نه ده ثبت شوې.", reviewerNote: "د کتنې یادښت او د پرېکړې دلیل", approve: "منل", reject: "ردول", requester: "غوښتونکی", details: "د غوښتنې معلومات", manageRights: "د سرچینې اجازه اداره کړئ", rightsVerified: "د سرچینې اجازه ثبت ده",
        }
      : {
          ...
          empty: "No records yet.", notCreated: "Listing not created yet", waiting: "The transfer was received, but it is not a public listing yet. After review and successful conversion, its listing link will appear here.", live: "Checking the new-listing queue every 30 seconds", configureWebhook: "Securely connect bot to production", reviewTransfer: "Review transfer", openListing: "Open listing", openSource: "Open original source", source: "Source", status: "Status", created: "Created", method: "Method", platform: "Platform", rows: "Rows", result: "Result", dryRun: "Dry run", liveRun: "Live run", accepted: "Accepted", rejected: "Rejected", errors: "Errors", row: "Row", sourceItem: "Source item", category: "Category", location: "Location", price: "Price", missingCategory: "Not assigned", missingLocation: "Not resolved", missingPrice: "Not normalized", activeSource: "Source enabled", emergencyStop: "Emergency stop enabled", reason: "Reason", score: "Score", latest: `Showing the latest ${RECENT_LIMIT} records`, claimsEmpty: "No ownership requests.", removalsEmpty: "No removal requests.", duplicatesEmpty: "No duplicate cases.", optOutsEmpty: "No opt-outs recorded.", reviewerNote: "Review note and decision reason", approve: "Approve", reject: "Reject", requester: "Requester", details: "Request details", manageRights: "Manage source rights", rightsVerified: "Source rights recorded",
        };
  ...
  const [sources, jobs, candidates, claims, removals, duplicates, optOuts, external, stale, sourcesResult, jobsResult, candidatesResult, claimsResult, removalsResult, duplicatesResult, optOutsResult, externalResult] = await Promise.all([
    ...
    supabase.from("listing_sources").select("id,name,source_type,platform,ingest_method,status,kill_switch_enabled,permission_record_id,created_at").order("created_at", { ascending: false }).limit(RECENT_LIMIT),
  ...
  return (
    <main className="mx-auto w-full max-w-7xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mt-3 flex flex-wrap items-center gap-3">
        <InventoryLiveRefresh label={copy.live} />
        <form action={configureTelegramImportWebhookAction}>
          <button className="rounded-xl border border-[var(--line)] bg-white px-3 py-2 text-xs font-bold hover:border-[var(--accent)]">
            {copy.configureWebhook}
          </button>
        </form>
      </div>
