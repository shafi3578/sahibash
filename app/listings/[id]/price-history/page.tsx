    ...
    return (
      <main className="mx-auto w-full max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
        <div className="rounded-lg border border-[var(--line)] bg-white p-6 text-center">
          <p className="text-[var(--ink-2)]">{ui.priceHistory.notFoundOrUnauthorized}</p>
          <Link
            href="/dashboard/my-ads"
            className="mt-3 inline-block text-[var(--accent)] font-semibold"
          >
            {ui.priceHistory.backToMyListings}
          </Link>
        </div>
      </main>
    );
  ...
  return (
    <main className="mx-auto w-full max-w-4xl px-4 py-8 sm:px-6 lg:px-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          <h1 className="font-display text-3xl font-bold">{ui.priceHistory.priceHistory}</h1>
      ...
      <div className="rounded-lg border border-[var(--line)] bg-white p-6">
        {!priceHistory || priceHistory.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-[var(--ink-2)]">{ui.priceHistory.noPriceChangesRecorded}</p>
          </div>
        ) : (
              ...
              <thead>
                <tr className="border-b border-[var(--line)]">
                  <th className="pb-3 text-left text-sm font-semibold text-[var(--ink-1)]">
                    {ui.priceHistory.date}
                  </th>
                  <th className="pb-3 text-left text-sm font-semibold text-[var(--ink-1)]">
                    {ui.priceHistory.oldPrice}
                  </th>
                  <th className="pb-3 text-left text-sm font-semibold text-[var(--ink-1)]">
                    {ui.priceHistory.newPrice}
                  </th>
                  <th className="pb-3 text-left text-sm font-semibold text-[var(--ink-1)]">
                    {ui.priceHistory.change}
                  </th>
                  <th className="pb-3 text-left text-sm font-semibold text-[var(--ink-1)]">
                    {ui.priceHistory.reason}
                  </th>
                </tr>
              </thead>
              <tbody>
