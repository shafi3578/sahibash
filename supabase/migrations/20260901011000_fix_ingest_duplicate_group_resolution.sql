-- Candidate duplicate_group_id references listing_duplicate_groups, not another
-- candidate. Create or reuse the group atomically and retain the canonical
-- candidate identity as evidence until that candidate is published.
...
declare
  ...
  v_canonical public.listing_ingest_candidates%rowtype;
...
begin
  ...
  if v_next_status = 'duplicate' then
    ...
    from public.listing_ingest_candidates canonical
    where canonical.id = p_duplicate_group_id
      and canonical.source_id is not distinct from v_candidate.source_id
    for update;
    if not found then raise exception 'The canonical duplicate candidate is invalid'; end if;

    v_resolved_group_id := v_canonical.duplicate_group_id;
    if v_resolved_group_id is null then
      insert into public.listing_duplicate_groups (
        canonical_listing_id, confidence, score, evidence, status, created_by
      ) values (
        v_canonical.candidate_listing_id,
        'exact',
        1,
        jsonb_build_object(
          'canonical_candidate_id', v_canonical.id,
          'source_id', v_canonical.source_id,
          'resolution', 'administrator_review'
        ),
        'open',
        p_actor_id
      ) returning id into v_resolved_group_id;

      update public.listing_ingest_candidates
      set duplicate_group_id = v_resolved_group_id, updated_at = now()
      where id = v_canonical.id;
    end if;
  elsif p_duplicate_group_id is not null then
    raise exception 'Rejected candidates cannot reference a duplicate candidate';
  end if;

  update public.listing_ingest_candidates
  set status = v_next_status,
      duplicate_group_id = case when v_next_status = 'duplicate' then v_resolved_group_id else null end,
  ...
  insert into public.listing_provenance_events (
    ...
    jsonb_build_object(
      ...
      'duplicate_group_id', case when v_next_status = 'duplicate' then v_resolved_group_id else null end,
      'canonical_candidate_id', case when v_next_status = 'duplicate' then p_duplicate_group_id else null end
    ),
    v_reason,
    v_candidate.job_id
  );
...
create or replace function private.sync_ingest_duplicate_group_canonical()
returns trigger
language plpgsql
security definer
set search_path = ''
...
begin
  if new.status = 'published'
     and new.candidate_listing_id is not null
     and new.duplicate_group_id is not null then
    update public.listing_duplicate_groups
    set canonical_listing_id = coalesce(canonical_listing_id, new.candidate_listing_id),
        updated_at = now()
...
drop trigger if exists sync_ingest_duplicate_group_canonical on public.listing_ingest_candidates;
create trigger sync_ingest_duplicate_group_canonical
after update of status, candidate_listing_id on public.listing_ingest_candidates
for each row execute function private.sync_ingest_duplicate_group_canonical();
