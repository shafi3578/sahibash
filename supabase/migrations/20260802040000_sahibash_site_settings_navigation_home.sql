...
alter table public.site_settings
  add column if not exists home_hero_title text not null default 'Discover trusted listings across Afghanistan',
  add column if not exists home_hero_subtitle text not null default 'Buy, sell, and browse with local-first controls and multilingual search.',
  add column if not exists home_primary_cta_label text not null default 'Browse listings',
  add column if not exists home_primary_cta_path text not null default '/listings',
  add column if not exists home_secondary_cta_label text default 'Post an ad',
  add column if not exists home_secondary_cta_path text default '/post-ad/create?posting=sell',
  add column if not exists navigation_links jsonb not null default '[{"label":"Listings","path":"/listings"},{"label":"Categories","path":"/categories"}]'::jsonb;
